...
body{{background:#07070f;font-family:system-ui,-apple-system,sans-serif;color:#e0e0e0;padding-bottom:40px}}
.card{{background:#0d0d1a;border:1px solid #1a1a2e;border-radius:12px;padding:16px;margin-bottom:14px}}
.section-card{{background:#0d0d1a;border-radius:14px;overflow:hidden;margin-bottom:14px}}
.metric{{flex:1 1 70px;min-width:70px;background:#0d0d1a;border:1px solid #1a1a2e;border-radius:10px;padding:10px 8px;text-align:center}}
.metric-label{{font-size:10px;color:#555;margin-bottom:4px;letter-spacing:.05em}}
.metric-value{{font-size:15px;font-weight:800;font-family:monospace;color:#e0e0e0}}
.cond-row{{display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid #1a1a2e}}
...
.score-bar-bg{{background:#1a1a2e;border-radius:4px;height:8px;width:100%;overflow:hidden}}
...
function condRow(label, value, ok, detail){{
  return `<div class="cond-row">
    ${{condDot(ok)}}
    <div style="flex:1">
      <div style="font-size:13px;font-weight:600;color:#e0e0e0">${{label}}</div>
      <div style="font-size:11px;color:#555;margin-top:1px">${{detail}}</div>
    </div>
    <div style="font-size:13px;font-weight:700;font-family:monospace;
      color:${{ok?"#00ff88":"#ff4444"}};min-width:60px;text-align:left">${{value}}</div>
  </div>`;
}}
...
function orderRow(label, price, note, color){{
  return `<div class="order-row" style="border:1px solid ${{color}}44">
    <div>
      <div style="font-size:12px;font-weight:600;color:#ccc;margin-bottom:3px">${{label}}</div>
...
function render(){{
  ...
  document.getElementById("earnings-btn").style.background    = earningsWeek ? "#7a2000" : "#0d2818";
  document.getElementById("earnings-btn").style.borderColor   = earningsWeek ? "#ff6600" : "#00ff8844";
  document.getElementById("earnings-btn").style.color         = earningsWeek ? "#ff9955" : "#00cc66";
  ...
  document.getElementById("sell-header").style.background = `linear-gradient(135deg,${{sell.color}}11,transparent)`;
  ...
  document.getElementById("rebuy-header").style.background = `linear-gradient(135deg,${{rebuy.color}}11,transparent)`;
...
<!-- HEADER -->
<div style="background:linear-gradient(135deg,#0d0d1a,#12122a);border-bottom:1px solid #1a1a2e;padding:20px 20px 16px;display:flex;justify-content:space-between;align-items:flex-start">
  <div>
    <div style="font-size:10px;color:#444;letter-spacing:.2em;font-family:monospace;margin-bottom:6px">TSLA · NASDAQ</div>
    <div style="font-size:22px;font-weight:800;color:#fff;letter-spacing:-.02em">Trade Decision Engine</div>
    <div style="font-size:11px;color:#444;margin-top:4px">&#128344; {generated_at}</div>
  </div>
  <div style="display:flex;flex-direction:column;align-items:center;gap:8px">
    <div style="background:#0d2818;border:1px solid #00ff8844;border-radius:10px;padding:8px 14px;text-align:center">
      <div style="font-size:9px;color:#00aa55;letter-spacing:.15em;margin-bottom:2px">HOLDING</div>
      <div style="font-size:22px;font-weight:800;color:#00ff88;font-family:monospace">{shares}</div>
...
<div style="padding:16px 16px 0">
  ...
  <button id="earnings-btn" onclick="toggleEarnings()"
    style="width:100%;padding:12px;border-radius:12px;border:2px solid;font-size:14px;font-weight:700;cursor:pointer;margin-bottom:14px;transition:all .2s">
  </button>

  <!-- MARKET DATA -->
  <div class="card">
    <div style="font-size:11px;color:#444;margin-bottom:12px;font-weight:600;letter-spacing:.1em">&#128202; נתוני שוק &mdash; {generated_at}</div>
    <div style="display:flex;gap:6px;flex-wrap:wrap">
      <div class="metric" style="background:#7070ff11;border-color:#7070ff44">
        <div class="metric-label">מחיר נוכחי</div>
        <div class="metric-value" style="color:#7070ff">${price:.2f}</div>
      </div>
      <div class="metric"><div class="metric-label">פתיחה</div><div class="metric-value">${op:.2f}</div></div>
      <div class="metric" style="background:{'#00ff8811' if pct>=3 else ''};border-color:{'#00ff8844' if pct>=3 else '#1a1a2e'}">
        <div class="metric-label">מהפתיחה</div>
        <div class="metric-value" style="color:{'#00ff88' if pct>=3 else '#e0e0e0'}">{pct:+.2f}%</div>
      </div>
      <div class="metric"><div class="metric-label">VWAP</div><div class="metric-value">${vwap:.2f}</div></div>
      <div class="metric" style="background:{'#ffaa0011' if rsi>=65 else ''};border-color:{'#ffaa0044' if rsi>=65 else '#1a1a2e'}">
        <div class="metric-label">RSI</div>
        <div class="metric-value" style="color:{'#ffaa00' if rsi>=65 else '#e0e0e0'}">{rsi}</div>
      </div>
  ...
  <!-- SELL SECTION -->
  <div id="sell-wrap" class="section-card" style="border:2px solid #44444400">
    <div id="sell-header" style="padding:18px 18px 14px;border-bottom:1px solid #1a1a2e">
      <div id="sell-action" style="font-size:22px;font-weight:800;margin-bottom:4px"></div>
      <div id="sell-sub" style="font-size:13px;color:#888;margin-bottom:12px"></div>
      <div style="display:flex;justify-content:space-between;margin-bottom:8px">
        <span style="font-size:11px;color:#555">ציון ביטחון</span>
    ...
    <div style="padding:14px 18px;border-top:1px solid #1a1a2e">
      <div style="font-size:11px;font-weight:700;color:#555;margin-bottom:10px">&#128203; פקודות Limit מוצעות</div>
      {f'<div class="order-row" style="border:1px solid #ff444444"><div><div style="font-size:12px;font-weight:600;color:#ccc;margin-bottom:3px">Limit Sell &mdash; 212 מניות (50%)</div><div style="font-size:11px;color:#555">+4% מהפתיחה | רווח משוער: ${profit4:.0f}</div></div><div style="font-size:18px;font-weight:800;font-family:monospace;color:#ff4444">${t4}</div></div>'}
      {f'<div class="order-row" style="border:1px solid #ff666644"><div><div style="font-size:12px;font-weight:600;color:#ccc;margin-bottom:3px">Limit Sell &mdash; 106 מניות (25%)</div><div style="font-size:11px;color:#555">+6% מהפתיחה | רווח משוער: ${profit6:.0f}</div></div><div style="font-size:18px;font-weight:800;font-family:monospace;color:#ff6666">${t6}</div></div>'}
    </div>
  </div>

  <!-- REBUY SECTION -->
  <div id="rebuy-wrap" class="section-card" style="border:2px solid #44444400">
    <div id="rebuy-header" style="padding:18px 18px 14px;border-bottom:1px solid #1a1a2e">
      <div id="rebuy-action" style="font-size:22px;font-weight:800;margin-bottom:4px"></div>
      <div id="rebuy-sub" style="font-size:13px;color:#888;margin-bottom:12px"></div>
      <div style="display:flex;justify-content:space-between;margin-bottom:8px">
        <span style="font-size:11px;color:#555">ציון ביטחון</span>
    ...
    <div style="padding:14px 18px;border-top:1px solid #1a1a2e">
      <div style="font-size:11px;font-weight:700;color:#555;margin-bottom:10px">&#128203; פקודות Limit מוצעות</div>
      {f'<div class="order-row" style="border:1px solid #00ff8844"><div><div style="font-size:12px;font-weight:600;color:#ccc;margin-bottom:3px">Limit Buy אגרסיבי (תוך-יומי)</div><div style="font-size:11px;color:#555">2% מתחת למחיר | +{extra} מניות בונוס</div></div><div style="font-size:18px;font-weight:800;font-family:monospace;color:#00ff88">${rb1}</div></div>'}
      {f'<div class="order-row" style="border:1px solid #00cc6644"><div><div style="font-size:12px;font-weight:600;color:#ccc;margin-bottom:3px">Limit Buy מאוזן (תוך-יומי)</div><div style="font-size:11px;color:#555">1.5% מתחת | איזון בין מחיר למילוי</div></div><div style="font-size:18px;font-weight:800;font-family:monospace;color:#00cc66">${rb15}</div></div>'}
      {f'<div class="order-row" style="border:1px solid #00994444"><div><div style="font-size:12px;font-weight:600;color:#ccc;margin-bottom:3px">Limit Buy שמרני (יום המחרת)</div><div style="font-size:11px;color:#555">3% מתחת | סיכוי מילוי נמוך אך מחיר טוב</div></div><div style="font-size:18px;font-weight:800;font-family:monospace;color:#009944">${rb2}</div></div>'}
    </div>
  </div>
